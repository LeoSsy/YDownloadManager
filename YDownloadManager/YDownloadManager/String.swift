...
extension String {
    ...
    /// 获取指定文件的大小
    public func fileSize()->NSInteger {
        if !FileManager.default.fileExists(atPath: self) {return 0}
        var size:NSInteger = 0
        do {
            size = try FileManager.default.attributesOfItem(atPath: self)[FileAttributeKey.size] as! NSInteger
        }catch {
            size = 0
        }
        return size
    ...
    /// 获取系统空闲的内存空间
    public static func systemfreeSize()->NSInteger {
        var size:NSInteger = 0
        do {
           size = try FileManager.default.attributesOfFileSystem(forPath: documentDir())[FileAttributeKey.systemFreeSize] as! NSInteger
        }catch {
            size = 0
        }
        return size
    ...
    /// 获取document 目录
    public static func documentDir()->String {
    ...
    /// 添加caches 目录
    public func addCachePath()->String {
        return (NSSearchPathForDirectoriesInDomains(FileManager.SearchPathDirectory.cachesDirectory, FileManager.SearchPathDomainMask.userDomainMask, true).last?.appending("/"+self))!
    }
